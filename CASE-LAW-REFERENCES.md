# Reviewed case-law references

This repository also contains a small set of separately reviewed case-law research references. These are distinct from the rule-change archive dataset and do not become controlling law or replace the complete reasons in the cited judgments.

## Canadian Immigration Misrepresentation: Procedural Fairness, Disclosure and Materiality

**Version:** 1.0  
**Published:** 2026-09-15  
**Research cutoff:** 2026-09-15  
**Human Legal QA:** APPROVED — RCIC Usman Khalil

Public access:
- `federal-court-misrepresentation-reference-v1.0.html` — human-readable public access layer
- `federal-court-misrepresentation-reference-v1.0.md` — canonical reviewed text

Dataset and citation/use companions:
- `federal-court-misrepresentation-reference-v1.0.csv` — tabular public dataset of the seven Human-Legal-QA-approved Federal Court reference rows
- `federal-court-misrepresentation-reference-v1.0.json` — machine-readable citation, source and case metadata
- `federal-court-misrepresentation-reference-v1.0.bib` — BibTeX citation record
- `federal-court-misrepresentation-reference-v1.0.ris` — RIS citation record for reference-manager import

The reference contains seven Federal Court decisions selected to preserve materially different reasoning and outcomes on procedural fairness, disclosure and A40(1)(a) materiality. It includes paragraph pinpoints, comparator controls, current statutory/policy anchors, methodology, later-treatment safeguards and explicit limits on what the table does not establish.

Suggested citation:

> MAK Canadian Immigration Services. *Canadian Immigration Misrepresentation: Procedural Fairness, Disclosure and Materiality — A Federal Court Reference Table*. Version 1.0, 2026-09-15. Reviewed by RCIC Usman Khalil.

## Use boundary

These materials are research references, not legal advice, not prediction tools and not exhaustive catalogues. Before relying on a proposition in a live matter, verify the current legislation, regulations, official guidance, complete reasons and current treatment of the cited decision.
